...
const sliderGallery = document.querySelector(".slider-gallery") as HTMLElement;

if (sliderGallery) {
    const slides = sliderGallery.querySelectorAll(".splide__slide") as NodeListOf<HTMLLIElement>;
    ...
    if (slides.length > 1) {
        (async () => {
            await import('@splideJs').then(async res => {
                ...
                const splide = sliderGallery.querySelector(".splide") as HTMLDivElement;
