...
export type RenderedComponentConstructor = {
    refresh: () => void,
    on: (event: string, done: () => void) => void,
    setup?(): void;
    mount(): void;
...
export type SplideComponentConstructor = {
    new(Splide: HTMLElement, options: Options): RenderedComponentConstructor;
}
...
export default class {
    ...
    private child: NodeListOf<HTMLElement>;
    private layout: undefined | string;
    private activeSlide: undefined | number;
    private sliderInstance: undefined | RenderedComponentConstructor;
    private sliderComponent: undefined | SplideComponentConstructor;
    private arrowSvgLayout: string;
    private closeSvgLayout: string;
    ...
    onClick(event) {
        ...
        const listItem = image.closest("[data-gallery-item]") as HTMLElement;
    ...
    onCloseClick(event) {
        ...
        if (button) {
            const parentGallery = button.closest(".gallery") as HTMLElement;
     ...
     * @param image
     */
    createImageGallery(image: HTMLImageElement): string | undefined {
        if (!image) return;
        ...
        if (this.child.length > 1) {
            ...
            const imageSrc = getElementsByData(images, "imageSource") as string[];
     ...
     * @param images
     */
    sliderGalleryGenerator(images: string[]) {
     ...
     */
    async initializerSliderGallery(activeIndex: number = 0) {
        const gallery = document.querySelector(".gallery") as HTMLDialogElement;

        if (gallery) {
            const slider = gallery.querySelector(".gallery__slider") as HTMLElement;
